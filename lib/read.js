...
exports.mdLinks = exports.onlyStatusLinks = exports.onlyStats = exports.onlyBrokesLinks = exports.readMd = exports.readDir = exports.getArrayOfObjectsLinks = void 0;
...
var readDir = function readDir(absolutePath) {
  return _fs["default"].readdir(absolutePath, function (err, data) {
    /* 
    console.log(data);
    ...
    var files = data;
    var absoluteRoute; //let pathOfDir = path.resolve(absolutePath);

    for (var x in files) {
      absoluteRoute = _path["default"].join(absolutePath, files[x]);
      console.log(absoluteRoute);

      if (_fs["default"].statSync(absoluteRoute).isDirectory() === true) {
        return readDir(absoluteRoute);
      } else if (_path["default"].extname("".concat(absoluteRoute)) == ".md") {
      ...
      if (stats === undefined) {
        ...
        if (stats.isFile()) {
          ...
          if (_path["default"].extname("".concat(absolutePath)) == ".md") {
            console.log("es md");
            var result = getArrayOfObjectsLinks(_fs["default"].readFileSync(absolutePath).toString(), absolutePath);
            console.log(result);
        ...
        if (options.validate && options.stats) {
          ...
          if (options.validate && options.stats) {
            resolve(onlyStatusLinks(arrayLink).then(function (data) {
              console.log(onlyBrokesLinks(data));
            }));
          }
