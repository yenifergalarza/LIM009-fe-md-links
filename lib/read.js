...
var readDir = function readDir(absolutePath) {
  ...
  _fs["default"].readdir(absolutePath, function (err, data) {
    var files = data; //let pathOfDir = path.resolve(absolutePath);
    ...
    try {
      for (var _iterator = files[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var x = _step.value;
        absoluteRoute = _path["default"].join(absolutePath, x);
  ...
  for (var _i = 0, _pathArray = pathArray; _i < _pathArray.length; _i++) {
    var link = _pathArray[_i];
    return readMd(link);
  }
};
